...
class TestEigendIntegration:
    ...
    def test_eigend_string_assertion_prevention(self, piw_session):
        ...
        def test_eigend_patterns(session_ctx):
            ...
            try:
                ...
                path_strings = [
                    "~/Library/Eigenlabs/2.3.0-community/Global",  # User home relative
                    "tmp/plugins",  # Project relative
    ...
    def test_eigend_crash_reproduction_attempt(self):
        ...
        # ~/Library/Eigenlabs/2.3.0-community/Global/current_setup-main
...
@pytest.mark.integration
class TestClipManagerIntegration:
    """
    Test clip_manager plugin in full application context.
    """
    
    @pytest.mark.integration
    @pytest.mark.timeout(15)
    def test_clip_manager_agent_in_application_context(self, piw_session):
        """Test that ClipManager Agent can be instantiated in application context."""
        def test_in_application(session_ctx):
            try:
                import sys
        ...
        results = piw_session['run'](test_in_application)
    ...
    @pytest.mark.integration
    @pytest.mark.timeout(15)
    def test_clip_manager_widget_functionality_in_application_context(self, eigend_agentd_session):
        ...
        if not context.get('success', False):
            pytest.skip(f"Agentd setup failed: {context.get('error', 'Unknown error')}")
        
        # Test completed successfully - run assertions
        agent = context['agent']
        widget = agent[1]  # Get the widget from the agent
        
        assert widget is not None, "Widget should exist"
        assert type(widget).__name__ == 'ClipManagerWidget', f"Widget should be ClipManagerWidget, got {type(widget).__name__}"
        assert hasattr(widget, '_ClipManagerWidget__widget'), "Widget should have native clip manager"