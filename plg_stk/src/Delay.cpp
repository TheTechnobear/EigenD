...
void Delay :: fill(StkFloat value)
{
    for (unsigned int i=0; i<inputs_.size(); i++)
      inputs_[i] = value;
    outputs_[0] = value;
...
void Delay :: setContentsAt(unsigned long tapDelay, StkFloat value)
{
    long tap = inPoint_ - tapDelay;
...
StkFloat Delay :: getDelay(void) const
{
  return delay_;
}
