# agent for generic keyboard controller based on mididevice
...
class Agent(agent.Agent):
    def __init__(self, address, ordinal):
        #
        agent.Agent.__init__(self, signature=version, names='keyboard controller', ordinal=ordinal)
        ...
        self[3][1]  = atom.Atom(domain=domain.BoundedInt(0,16),init=1,policy=atom.default_policy(self.__channel),names='midi channel')

        self.device.channel(self[3][1].get_value())
        ...
        self.device.set_cc_map(PEDAL_3,11) # expression 
        self.device.set_cc_map(PEDAL_4,4)  # foot controller
