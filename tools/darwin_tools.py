...
class PiDarwinEnvironment(unix_tools.PiUnixEnvironment):
    ...
    def __init__(self,platform):
        ...
        if platform == 'macosx-x86-64' :
            self.Append(CXXFLAGS=Split('-std=c++17 -Wno-c++11-narrowing -Wno-inconsistent-missing-override -Wno-deprecated-register'))
            ...
            self.Append(CXXFLAGS=Split('-std=c++17 -stdlib=libc++ -Wno-c++11-narrowing -Wno-inconsistent-missing-override'))
            ...
            self.Append(CXXFLAGS=Split('-std=c++17 -Wno-c++11-narrowing -Wno-inconsistent-missing-override'))
            self.Append(CCFLAGS=Split('-arch i386  -msse3 -DDEBUG_DATA_ATOMICITY_DISABLED -DPI_PREFIX=\\"$PI_PREFIX\\" -mmacosx-version-min=10.13'))
            self.Append(LINKFLAGS=Split('-arch i386 -framework Accelerate -Wl,-rpath,@executable_path/ -mmacosx-version-min=10.13'))
