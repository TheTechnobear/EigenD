...
debug_file  = '/tmp/parser.out'# Default name of the debugging file
    ...
    for I in C:
        ...
        for p in I:
            try:
                ...
                raise YaccError("Hosed in slr_parse_table") from e
        ...
        for j in J:
            ...
            if jlr_index < len(jprod) - 1 and jprod[jlr_index+1] in Nonterminals:
                ...
                for x in j.lrafter:
                    ...
                    # Add B --> .G to J
                    if x.lr_next.setnumber in lookaheads:
        ...
        if len(p.prod) > 0:
            if p.prod[0] in Nonterminals:
                if p.prod[0] not in reducednonterminals:
                    ...
                    if p.prod[0] != nt:
                        if not p.prod[0] in NTReductions:
...
def yacc(method=default_lr, debug=yaccdebug, module=None, tabmodule=tab_module, start=None, check_recursion=1, optimize=0,write_tables=0,debugfile=debug_file):
    global yaccdebug
    yaccdebug = debug
