...
def natsort_key(s):
    """Natural sort key for Python 3"""
    parts = re.findall(r'(\d+|\D+)', s)
    result = []
    for part in parts:
        try:
            # Convert to int if possible, use tuple (type, value, original) for stable sorting
            num = int(part)
...
def get_detected_setup():
    ...
    if rd is not None:
        ...
        fs = [s for s in rs if filter_valid_setup(s)]
        fs.sort(key=lambda x: natsort_key(x), reverse=False)
...
def get_setup_slot(slot):
    ...
    for (sp,sd,sn) in resource.safe_walk(rd):
        for s in [s for s in sn if filter_valid_setup(s)]:
            s3 = upgrade.split_setup(s)
            if s3[1]==slot:
...
def find_user_setups_flat():
    ...
    # FIXED: Use predicate constructor for structured term
    t = piw.term("tree", 0)  # "tree" is a predicate name
    t.add_arg(-1, piw.term(piw.makestring('user setups', 0)))

    for (sp,sd,sn) in resource.safe_walk(rd):
        for s in [s for s in sn if filter_valid_setup(s)]:  
            # FIXED: Use predicate constructor for structured term
            t3 = piw.term('leaf', 0)  # "leaf" is a predicate name
            s3 = upgrade.split_setup(s)
            # FIXED: Use data constructor for string values
            name = piw.term(piw.makestring(urllib_unquote(s3[0]), 0)) if s3[0] else piw.term()
...
class Menu:
    ...
    def term(self):
        ...
        k = list(self.children2.keys())
        k.sort(key=natsort_key)
...
def find_user_setups():
    ...
    for (sp,sd,sn) in resource.safe_walk(rd):
        # Convert filter to list to avoid iterator issues
        valid_setups = [s for s in sn if filter_valid_setup(s)]
...
def find_old_setups():
    ...
    for v in resource.find_installed_versions(filter_upgradeable_version):
        ...
        for (sp,sd,sn) in resource.safe_walk(rd):
            sd=()
            sv = [s for s in sn if filter_valid_setup(s)]
            if sv:
                m2 = Menu(v)
                for s in sv:
                    s3 = upgrade.split_setup(s)
...
def find_all_setups():
    try:
        m = Menu('Setups')
        user_result = find_user_setups()
        m.add_child(user_result)
...
class Agent(agent.Agent):
    ...
    def __createverb(self,subject,plugin):
        ...
        if not address:
            return [action.error_return(m,plugin,'create') for m in delegate.errors]
